...
int axel_open(axel_t *axel) {
	...
	if (axel->outfd == -1) {
		...
		if ((axel->outfd = open(axel->filename, O_CREAT | O_WRONLY, 0666))
				== -1) {
			axel_message(axel, _("Error opening local file"));
			return (0);
		}
...
void axel_do(axel_t *axel) {
				...
				if (size == -1) {
					...
					if (axel->conf->verbose) {
						...
						if (axel->conn[i].currentbyte < axel->conn[i].lastbyte
								&& axel->size != INT_MAX) {
							axel_message(axel,
									_("Connection %i unexpectedly closed"), i);
						} else {
	...
	for (i = 0; i < axel->conf->num_connections; i++) {
		if (!axel->conn[i].enabled && axel->conn[i].currentbyte
	...
	axel->bytes_per_second = (int) ((double) (axel->bytes_done
			- axel->start_byte) / (gettime() - axel->start_time));
	axel->finish_time = (int) (axel->start_time + (double) (axel->size
			- axel->start_byte) / axel->bytes_per_second);
...
static void axel_divide(axel_t *axel) {
	...
	for (i = 1; i < axel->conf->num_connections; i++) {
		...
		axel->conn[i].lastbyte = axel->conn[i].currentbyte + axel->size
