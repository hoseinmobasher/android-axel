...
static inline size_t MAX_ROOM_NEEDED(const arguments *ap, size_t arg_index,
		FCHAR_T conversion, arg_type type, int flags, size_t width,
		int has_precision, size_t precision, int pad_ourselves) {
	size_t tmp_length;
...
#define CLEANUP() \
  ...
  if (a.arg)                                                            \
...

	{
		...
		size_t i;
		DIRECTIVE *dp;
		/* Output string accumulator.  */DCHAR_T *result;
		size_t allocated;
		size_t length;
...
# endif
					...
					/* Allocate a temporary buffer of sufficient size for calling
					 sprintf.  */
					tmp_length = MAX_ROOM_NEEDED(&a, dp->arg_index,
...
# endif
						...
						if (count < 0) {
							...
							if (buf_malloced != NULL)
								free(buf_malloced);
							CLEANUP ();
							errno = (saved_errno != 0 ? saved_errno
...
# endif
						...
						if (count > allocated - length) {
							/* Need at least count elements.  But allocate
							 proportionally.  */
							size_t n = xmax(xsum(length, count),
									xtimes(allocated, 2));
...
#undef prec_ourselves
		...
		/* Add the final NUL.  */ENSURE_ALLOCATION (xsum (length, 1));
		result[length] = '\0';

		if (result != resultbuf && length + 1 < allocated) {
			/* Shrink the allocated memory if possible.  */DCHAR_T *memory;
		...
		*lengthp = length;
		...
		out_of_memory: if (!(result == resultbuf || result == NULL))
			free(result);
		if (buf_malloced != NULL)
			free(buf_malloced);
		out_of_memory_1: CLEANUP ();
		errno = ENOMEM;
		return NULL;
	}
}
