...
# endif
	...
	/* See whether this is a SUID binary or not.  */DETERMINE_SECURE;
	...
	stpcpy(
			(char *) mempcpy(stpcpy(stpcpy(xdomainname, categoryname), "/"),
	...
	while (1) {
		...
		if (strcmp(single_locale, "C") == 0 || strcmp(single_locale, "POSIX")
				== 0)
			break;
		...
		if (domain != NULL) {
			...
			if (retval != NULL) {
				/* Found the translation of MSGID1 in domain DOMAIN:
				 starting at RETVAL, RETLEN bytes.  */FREE_BLOCKS (block_list);
				if (foundp == NULL) {
					...
					if (newp != NULL) {
						...
						new_domainname = (char *) mempcpy(newp->msgid.appended,
								msgid1, msgid_len);
						...
						foundp = (struct known_translation_t **) tsearch(newp,
								&root, transcmp);

						gl_rwlock_unlock(tree_lock);

						if (foundp == NULL || __builtin_expect(*foundp != newp,
								0))
							/* The insert failed.  */
							free(newp);
					}
	...
	return_untranslated:
	/* Return the untranslated MSGID.  */FREE_BLOCKS (block_list);
	gl_rwlock_unlock(_nl_state_lock);
#ifdef _LIBC
	__libc_rwlock_unlock (__libc_setlocale_lock);
#endif
	...
	if (!ENABLE_SECURE) {
		extern void _nl_log_untranslated(const char *logfilename,
				const char *domainname, const char *msgid1, const char *msgid2,
				int plural);
...
#endif
{
	...
	if (domain->hash_tab != NULL) {
		...
		while (1) {
			nls_uint32 nstr = W(domain->must_swap_hash_tab,
					domain->hash_tab[idx]);
			...
			if (nstr < nstrings ? W(domain->must_swap,
					domain->orig_tab[nstr].length) >= len && (strcmp(
					msgid,
