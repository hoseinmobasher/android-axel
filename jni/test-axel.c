...
#define LOG_TAG "Axel"
#define LOG_LEVEL 10
#define LOGI_(level, ...) if (level <= LOG_LEVEL) {__android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__);}
#define LOGE_(level, ...) if (level <= LOG_LEVEL) {__android_log_print(ANDROID_LOG_ERROR, LOG_TAG, __VA_ARGS__);}
...
static void print_alternate_output(axel_t *axel, JNIEnv * env, jclass this);
...
{ "max-speed", 1, NULL, 's' }, { "num-connections", 1, NULL, 'n' }, { "output",
		...
		{ "no-proxy", 0, NULL, 'N' }, { "quiet", 0, NULL, 'q' }, { "verbose",
				0, NULL, 'v' }, { "help", 0, NULL, 'h' }, { "version", 0, NULL,
				'V' }, { "alternate", 0, NULL, 'a' },
		{ "header", 1, NULL, 'H' }, { "user-agent", 1, NULL, 'U' }, { NULL, 0,
				NULL, 0 } };
#endif

/* For returning string values from functions				*/
static char string[MAX_STRING];
static char retstr[MAX_STRING];
...
JNIEXPORT void JNICALL Java_com_example_axel_MainActivity_nativeCancel(
		JNIEnv * env, jclass this, jstring link)
{
	run = 0;
	LOGI_(10, "Running stopped");
}

JNIEXPORT jint JNICALL Java_com_example_axel_MainActivity_nativeDownload(
		JNIEnv * env, jclass this, jobjectArray argv_s) {
	//Methods Signature Console Command: javap -s -p MainActivity

	/*************‌ SETUP Java TestCall Method ****************/
	cls = (*env)->FindClass(env, "com/example/axel/MainActivity");
	testCall = (*env)->GetMethodID(env, cls, "messageMe",
			"(Ljava/lang/String;)V");
	/***************** FINISH *********************/

	jsize argc = (*env)->GetArrayLength(env, argv_s);
	char * argv[argc];
	int looper = 0;

	for (; looper < argc; looper++) {
		jstring string = (jstring)(*env)->GetObjectArrayElement(env, argv_s,
				looper);
		argv[looper] = (*env)->GetStringUTFChars(env, string, 0);
	...
	while (1) {
		...
		case 'q':
			...
			if (open("/dev/null", O_WRONLY) != 1) {
				LOGI(10, "Can't redirect stdout to /dev/null.");
	...
	if (argc - optind == 0) {
		...
		if (scanf("%1024[^\n]s", s) != 1) {
			...
			LOGI(10, "Error when trying to read URL (Too long?).");
			return (1);
		}
	} else {
		s = argv[optind];
		if (strlen(s) > MAX_STRING) {
			LOGI(10, "Can't handle URLs of length over %d\n",
					MAX_STRING);
	...
	for (looper = 0; looper < argc; looper++)
		(*env)->DeleteLocalRef(env, argv[looper]);

	(*env)->DeleteLocalRef(env, argv);

	LOGI(10, "Initializing download: %s", s);
	sprintf(retstr, "init=%s", s);
	jretstr = (*env)->NewStringUTF(env, retstr);
	(*env)->CallVoidMethod(env, this, testCall, jretstr);
	(*env)->DeleteLocalRef(env, jretstr);
	...
	if (do_search) {
		...
		search[0].conf = conf;
		if (conf->verbose)
			LOGI(10, "Doing search...");
		i = search_makelist(search, s);
		if (i < 0) {
			LOGI(10, "File not found");
			fprintf(stderr, _("File not found\n"));
			return (1);
		}
		if (conf->verbose)
			LOGI(10, "Testing speeds, this can take a while...");
		j = search_getspeeds(search, i);
		search_sortlist(search, i);
		if (conf->verbose) {
			LOGI(10,"%i usable servers found, will use these URLs:\n", j);
	...
	if (*fn) {
		...
		if (access(fn, F_OK) == 0)
			if (access(string, F_OK) != 0) {
				...
				LOGI(10, "No state file, cannot resume!\n");
				return (1);
			}
		if (access(string, F_OK) == 0)
			if (access(fn, F_OK) != 0) {
				LOGI(10, "State file found, but no downloaded data. Starting from scratch.");
				unlink(string);
			}
	...
	while (!axel->ready && run) {
		...
		if (conf->alternate_output) {
			if (!axel->message && prev != axel->bytes_done)
				print_alternate_output(axel, env, this);
		} else {
				...
				for (i = 0; i < done; i++) {
					...
					if ((i % 50) == 0) {
						...
						if (axel->size < 10240000) {
							LOGI(10, "\n[%3lld%%]  ",
									min(100, 102400 * i / axel->size));
						} else {
							LOGI(10, "\n[%3lld%%]  ",
									min(100, i / (axel->size / 102400)));
						}
					} else if ((i % 10) == 0) {
						//putchar(' ');
					}
	...
	sprintf(retstr, "\nDownloaded %s in %s. (%.2f KB/s)\n",
			string + MAX_STRING / 2, time_human(gettime() - axel->start_time),
			(double) axel->bytes_per_second / 1024);
	jretstr = (*env)->NewStringUTF(env, retstr);
	(*env)->CallVoidMethod(env, this, testCall, jretstr);
	(*env)->DeleteLocalRef(env, jretstr);
...
char *time_human(int value) {
	if (value == 1)
		...
		sprintf(string, _("%i:%02i:%02i seconds"), value / 3600,
...
static void print_alternate_output(axel_t *axel, JNIEnv * env, jclass this) {
	...
	LOGI(10,"\n[%3ld%%] [", min(100, (long) (done * 100. / total + .5)));
	sprintf(retstr, "percent=%3ld%%",
			min(100, (long) (done * 100. / total + .5)));
	jretstr = (*env)->NewStringUTF(env, retstr);
	(*env)->CallVoidMethod(env, this, testCall, jretstr);
	(*env)->DeleteLocalRef(env, jretstr);
	...
	if (axel->bytes_per_second > 1048576) {
		LOGI(10, "] [%6.1fMB/s]", (double) axel->bytes_per_second / (1024 * 1024));
		sprintf(retstr, "speed=%6.1fMB/s",
				(double) axel->bytes_per_second / (1024 * 1024));
		jretstr = (*env)->NewStringUTF(env, retstr);
		(*env)->CallVoidMethod(env, this, testCall, jretstr);
		(*env)->DeleteLocalRef(env, jretstr);
	} else if (axel->bytes_per_second > 1024) {
		LOGI(10, "] [%6.1fKB/s]", (double) axel->bytes_per_second / 1024);
		sprintf(retstr, "speed=%6.1fKB/s",
				(double) axel->bytes_per_second / 1024);
		jretstr = (*env)->NewStringUTF(env, retstr);
		(*env)->CallVoidMethod(env, this, testCall, jretstr);
		(*env)->DeleteLocalRef(env, jretstr);
	} else {
		LOGI(10, "speed=%6.1fB/s", (double) axel->bytes_per_second);
		sprintf(retstr, "speed=%6.1fB/s", (double) axel->bytes_per_second);
		jretstr = (*env)->NewStringUTF(env, retstr);
		(*env)->CallVoidMethod(env, this, testCall, jretstr);
		(*env)->DeleteLocalRef(env, jretstr);
	}
	...
	if (done < total) {
		...
		hours -= days * 24;
		if (days) {
			LOGI(10," [%2dd%2d]", days, hours);
			sprintf(retstr, "remain=%2dd%2d", days, hours);
			jretstr = (*env)->NewStringUTF(env, retstr);
			(*env)->CallVoidMethod(env, this, testCall, jretstr);
			(*env)->DeleteLocalRef(env, jretstr);
		} else if (hours) {
			LOGI(10," [%2dh%02d]", hours, minutes);
			sprintf(retstr, "remain=%2dh%02d", hours, minutes);
			jretstr = (*env)->NewStringUTF(env, retstr);
			(*env)->CallVoidMethod(env, this, testCall, jretstr);
			(*env)->DeleteLocalRef(env, jretstr);
		} else {
			LOGI(10," [%02d:%02d]", minutes, seconds);
			sprintf(retstr, "remain=%02d:%02d", minutes, seconds);
			jretstr = (*env)->NewStringUTF(env, retstr);
			(*env)->CallVoidMethod(env, this, testCall, jretstr);
			(*env)->DeleteLocalRef(env, jretstr);
		}
...
void print_help() {
#ifdef NOGETOPTLONG
	LOGI(10,"Usage: axel [options] url1 [url2] [url...]\n"
			"\n"
...
void print_version() {
	LOGI(10, "Axel version %s (%s)\n" , AXEL_VERSION_STRING, ARCH);LOGI(10, "\nCopyright 2001-2002 Wilmer van der Gaast.\n");
}
